...
//        $jsonEncode = executeMultiQueryArray($sql);
        $error = "คุณใช้สิทธิ์ครบแล้ว";
        writeToLog("validate fail: $error, file: " . basename(__FILE__) . ", user: " . $_POST["modifiedUser"]);
        $response = array('success' => "false", 'data' => null, 'error' => $error, 'status' => 2);
...
//            $currentDateTime = date('Y-m-d H:i:s');
            ...
            $selectedRow = getSelectedRow($sql);
            $luckyDrawTimeLimit = $selectedRow[0]["Value"];
            $sql = "select 'Try again next time' Header,4 RewardRank;";
...
//            $currentDateTime = date('Y-m-d H:i:s');
            ...
            $selectedRow = getSelectedRow($sql);
            $luckyDrawTimeLimit = $selectedRow[0]["Value"];
            $sql = "select rewardRedemption.Header, rewardRedemption.RewardRank, DiscountGroupMenuID, '$promoCode' VoucherCode, 1 ShowOrderNow from rewardRedemption where rewardRedemptionID = '$rewardRedemptionID';";
            $sql .= "select count(*) LuckyDrawCount from luckyDrawTicket left join receipt on luckyDrawTicket.receiptID = receipt.receiptID where luckyDrawTicket.memberID = '$memberID' and receipt.branchID = '$branchID' and rewardRedemptionID = -1 and TIME_TO_SEC(timediff('$currentDateTime', luckyDrawTicket.modifiedDate)) <= '$luckyDrawTimeLimit';";
//            echo "<br>" . $sql;//test
