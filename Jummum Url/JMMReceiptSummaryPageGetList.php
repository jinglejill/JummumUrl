<?php
    ...
    $sql = "select * from (select  (@row_number:=@row_number + 1) AS Num, receipt.* from receipt, (SELECT @row_number:=0) AS t where memberID = '$memberID' and status != 3 order by (HasBuffetMenu and (TIME_TO_SEC(timediff('$currentDateTime', ReceiptDate))<TimeToOrder))desc, ReceiptDate desc)a where Num > $perPage*($page-1) limit $perPage;";
